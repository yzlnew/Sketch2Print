...
- **先判断物理对象，不要照抄贴纸设计**：参考图中的白色描边、贴纸边、背景留白、阴影块、高光块，可能只是 2D 视觉设计，不一定要建成实体。建模前要明确哪些是可打印的 3D 结构，哪些只是图案/贴纸效果。
- **像素风对象应体块化**：像素图转 3D 时，优先用长方体/方块组合表达主体轮廓和关键组件；给模型足够深度，让它成为真实 3D 物件，而不是薄浮雕贴片。
- **表面细节克制使用**：眼睛、轮毂等确实属于模型特征的元素，可以做成前表面的浅凸/浅凹色块；纯阴影、渐变、装饰性暗面默认不要建模，除非用户明确要求做成浮雕图案。
- **组件关系用多视图检查**：复杂组件（如滑板 deck、truck、wheel）必须用正面、侧面、俯视等多个视角检查空间关系，避免零件互相穿插、冲突或不合理 overlap。
- **接触和重叠要有意图**：需要合并成一体打印的块可以用很小的 `eps` 接触/重叠保证流形；不同功能层级的组件应明确上下/前后位置，不要因为视觉拼块导致大面积穿模。
- **提交保持范围干净**：提交示例或 README 时，只引用本次已准备提交的 reference/render/model；不要让 README 指向未提交的其他迭代目录或临时实验成果。
